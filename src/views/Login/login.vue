...
export default {
  ...
  mounted () {
    // 从进件列表页面跳转过来
    if (this.info) {
      const userInfo = JSON.parse(this.info);
      this.formData.name = userInfo.name;
      this.formData.idno = userInfo.idNo;
    }
    // 第二次登陆，token过期，登陆验证
    if (this.userInfo) {
      const userInfo1 = JSON.parse(this.userInfo);
  ...
  methods: {
    ...
    isValidFormData () {
      ...
      const reg_name = /^[\u4e00-\u9fa5|·|•|.|●]+$/gi;
      // eslint-disable-next-line camelcase
      const reg_idNo = /^([1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3})|([1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|[X|x]))$/;
      // eslint-disable-next-line camelcase
      const reg_mobile = /^1\d{10}$/;
      // eslint-disable-next-line camelcase
      const reg_code = /^\d{4,8}$/;
